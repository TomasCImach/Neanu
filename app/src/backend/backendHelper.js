...
const getMetaplex = () => {
    ...
    const wallet = Keypair.fromSecretKey(Uint8Array.from(keyPairDevnet));
...
const updateMetadata = async (nftAddress, newMetadata) => {
    const metaplex = getMetaplex();
    ...
    const updateResponse = await metaplex
        .nfts()
        .update({
            nftOrSft: nft,
            uri: newUri
...
const originalMetadata = async (nftAddress) => {
    ...
    const {
        ...
        .update({
            nftOrSft: nft,
            uri: 'https://arweave.net/4AB26UU7oq3BEX_F0U84bJClIsJpVGnLCS_oXSt1-gY'
        })
        .run();
...
const createCache = () => {
    ...
    for (let i = 0; i < number; i++) {
        let num = i.toString()
        items[num] = {
            "name": "Naenu #" + (i + 1).toString(),
            "image_hash": "cf00d2024e96b795ffbaaf27c07e961a913a264c12aea360d2aa3761030ef5af",
            "image_link": "https://arweave.net/U6PeBGoTJtcIHf5zZ9Fpb367dcf-AZ4qgZ6qKnt4dss?ext=png",
            "metadata_hash": "308c5b5ff03ee7c4c1704e746822d40c69b8687beddec3ae357f5c04484a5a1c",
            "metadata_link": "https://arweave.net/Sv4s2wCNV-fdvTknJf7w7bdKCm71CaGMeB1y2B8BqCk",
            "onChain": true
        }
    }
...
const getProvider = () => {
    ...
    const connection = new Connection(rpcHost, 'finalized');
...
const getCandyMachineState = async () => {
    ...
    return ({
        ...
        state: {
            ...
            isActive: (presale || candyMachine.data.goLiveDate.toNumber() < new Date().getTime() / 1000) &&
...
const mintToken = async () => {
    ...
    const keyPairDevnet = process.env.REACT_APP_SOLANA_KEYPAIR.split("-");
    const walletAddress = Keypair.fromSecretKey(Uint8Array.from(keyPairDevnet));
    ...
    signers.push(candyMachine.program.provider.wallet)
    try {
        return {
            txId: (
                await sendTransactions(
                    ...
                    [signers, [candyMachine.program.provider.wallet]]
                )
            ).txs.map((t) => t.txid),
            mint: mint.publicKey
        };
